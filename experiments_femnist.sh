                ...
                for nb_micro_batches in "${micro_batches_per_iteration[@]}"; do
                    for lr in "${lrs[@]}"; do
                        for seed in "${seeds[@]}"; do
                            ...
                            cmd=(python -u -m MFDL.simulations.decentralized_simulation --nb_nodes "$nb_nodes" --lr "$lr" --num_repetition "$num_repetition" --nb_batches 16 --mu "$mu" --graph_name "$graph_name" --use_optimals $hyperparameter_flag $recompute_flag $pre_cache_flag --dataloader_seed "$seed" --dataset femnist --nb_micro_batches "$nb_micro_batches")
