            ...
            for graph_name in "${graph_names[@]}"; do
                for lr in "${lrs[@]}"; do
                    for seed in "${seeds[@]}"; do
                        ...
                        cmd=(python simulations/housing.py --nb_nodes $nb_nodes --lr $lr --num_repetition $num_repetition --nb_batches 16 --epsilon $epsilon --graph_name "$graph_name" --use_optimals $recompute_flag $pre_cache_flag --dataloader_seed $seed)
                        echo "${cmd[@]}":
                        setsid "${cmd[@]}" &
                        pid=$!
                        pids+=($pid)
